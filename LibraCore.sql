...
CREATE TABLE IF NOT EXISTS `BanSaoSach` (
	`id_BanSaoSach` INTEGER NOT NULL AUTO_INCREMENT,
	`id_Sach` INTEGER NOT NULL,
	`TinhTrangSach` ENUM('Tot', 'Hong', 'Mat') DEFAULT 'Tot',
	PRIMARY KEY(`id_BanSaoSach`)
);
...
CREATE TABLE IF NOT EXISTS `ChiTietPhieuMuon` (
	...
	`id_BanSaoSach` INTEGER NOT NULL,
	`NgayTra` DATE,
...
ON UPDATE CASCADE ON DELETE CASCADE;
ALTER TABLE `ChiTietPhieuMuon`
ADD FOREIGN KEY(`id_BanSaoSach`) REFERENCES `BanSaoSach`(`id_BanSaoSach`)
ON UPDATE CASCADE ON DELETE RESTRICT;
...
CREATE INDEX idx_bansao_sach ON BanSaoSach(id_Sach);

CREATE INDEX idx_ctpm_bansao ON ChiTietPhieuMuon(id_BanSaoSach);
CREATE INDEX idx_ctpm_phieumuon ON ChiTietPhieuMuon(id_PhieuMuon);

CREATE INDEX idx_phieumuon_the ON PhieuMuon(id_TheThanhVien);

CREATE INDEX idx_ctpn_phieunhap ON ChiTietPhieuNhap(id_PhieuNhap);
CREATE INDEX idx_ctpn_sach ON ChiTietPhieuNhap(id_Sach);

CREATE INDEX idx_ctpp_phieuphat ON ChiTietPhieuPhat(id_PhieuPhat);
CREATE INDEX idx_ctpp_ctpm ON ChiTietPhieuPhat(id_ChiTietPhieuMuon);

CREATE INDEX idx_sach_tensach ON Sach(TenSach);
CREATE INDEX idx_docgia_ten ON DocGia(TenDocGia);
CREATE INDEX idx_ctpm_trangthai ON ChiTietPhieuMuon(id_BanSaoSach, TinhTrangTra);
